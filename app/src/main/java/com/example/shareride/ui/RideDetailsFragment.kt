...
class RideDetailsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val recyclerView = view.findViewById<RecyclerView>(R.id.joined_users_recyclerview)
        ...
        fun refreshJoinedUsers(rideId: String) {
            ...
            rideRef.get().addOnSuccessListener { document ->
                if (document.exists()) {
                    ...
                    if (recyclerView.visibility == View.GONE && ride.joinedUsers.isNotEmpty()) {
                        recyclerView.visibility = View.VISIBLE
                    }

                    ride.joinedUsers.forEach { userId ->
                        userCollection.document(userId).get().addOnSuccessListener { userDoc ->
                            if (userDoc.exists()) {
                                val user = User.fromJSON(userDoc.data ?: emptyMap())
                                users.add(user)
                                if (users.size == ride.joinedUsers.size) {
                                    val adapter = JoinedUsersAdapter(users, onItemClick = { userId ->
                                    })
        ...
        if (rideId != null) {
            ...
            rideRef.get().addOnSuccessListener { document ->
                if (document.exists()) {
                    val ride = Ride.fromJSON(document.data ?: emptyMap())
                    joinedUsers = ride.joinedUsers.toMutableList() // Ensure joinedUsers is synced with Firestore
                    updateButtons(joinedUsers.contains(firebaseModel.getCurrentUserId()))

                    refreshJoinedUsers(rideId)
                }
            }
        }
    ...
    private fun getUserPictureUrl(userId: String, callback: (String) -> Unit) {
        FirebaseFirestore.getInstance().collection("users").document(userId).get()
            .addOnSuccessListener { document ->
                if (document.exists()) {
                    val user = User.fromJSON(document.data ?: emptyMap())
                    callback(user.pictureUrl)
                } else {
                    callback("")
                }
            }
